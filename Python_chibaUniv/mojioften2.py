...
jogai = "あいうえおかきくけこさしすせそたちつてとなにぬねのはひふへほ" \
        "まみむめもやゆよらりるれろわをん" \
        "がぎぐげござじずぜぞだぢづでどばびぶべぼぱぴぷぺぽ" \
        "ぁぃぅぇぉっゃゅょゎゐゑ、。"

def string_to_frequencies(string,jogai):
    myDict = {}
    for word in string:
        if word not in jogai:
            if word in myDict:
                myDict[word] +=1
            else:
                myDict[word]=1
...
bocchan = string_to_frequencies(text, jogai)
print(moji_often(bocchan, 3))